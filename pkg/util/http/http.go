...
func Post(url string, params map[string]interface{}, body string, headerOptions ...HeaderOption) (string, int, error) {
	...
	req.Header.Set(jaeger.TraceContextHeaderName, threadlocal.GetValue(consts.JaegerContextTraceKey))
	...
	logger.InfoF("http type: POST|request [%s] starting|request body [%s]|request headers [%s]", fullUrl, body, headers)
	...
	logger.InfoF("http type: POST| request [%s] successful| request body [%s]|request headers [%s]|response status code [%d]| response body [%s]|time-consuming [%s]", fullUrl, body, headers, httpCode, respBody, timeConsuming)
...
func Get(url string, params map[string]interface{}, headerOptions ...HeaderOption) (string, int, error) {
	...
	headers := json.ToJsonIgnoreError(req.Header)
	logger.InfoF("http type: GET|request [%s] starting|request headers [%s]", fullUrl, headers)
	...
	respBody, httpCode, err := responseHandle(resp, err)
	logger.InfoF("http type: GET| request [%s] successful|request headers [%s]|response status code [%d]|response body [%s]| time-consuming [%s]", fullUrl, headers, httpCode, respBody, timeConsuming)
	return respBody, httpCode, err
}
